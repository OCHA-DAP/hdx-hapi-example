<script>
    ...
    loadCSVSources(sources, 4)
      .then(allData => {
        ...
        if (allData[0].length>0) {
          ...
          getCountryMetadata(allData[0][0].resource_hdx_id, tabs[0].id);
        }
        if (allData[1].length>0) {
          formatOrgsData(allData[1].concat(allData[2]));
          getCountryMetadata(allData[1][0].resource_hdx_id, tabs[1].id);
        }
        if (allData[3].length>0) {
          formatHNOData(allData[3]);
          getCountryMetadata(allData[3][0].resource_hdx_id, tabs[2].id);
        }
        if (allData[4].length>0) {
          formatIPCData(allData[4]);
          getCountryMetadata(allData[4][0].resource_hdx_id, tabs[3].id);
        }
  ...
  async function getCountryMetadata(hdx_id, view) {
    // try {
    ...
    //   }
    //   metadata[view] = await metadataPromises[hdx_id];
    // } catch (error) {
  ...
  function formatPopulationData(data) {
    ...
    //   popData.dataset_hdx_id = data[0].resource_hdx_id;
    // }
    ...
    //if there are no population demographics
    if (ageData.length<=0) {
      ...
      ageData.malePopulation = totalMalePopulation/totalPopulation;
      ageData.femalePopulation = totalFemalePopulation/totalPopulation;
    }
    ...
    if (popData.totalValue===0) popData.totalValue = undefined;
  }
  ...
  function createKeyFigures(iso3) {
    ...
    loadCSVSources(sources, 2)
      .then(results => {
        ...
        let resource_id = (pop.length>0) ? pop[0].resource_hdx_id : 0;
        if (totalValue>0) updateKeyFigureData({title: 'Population', value: totalValue, valueFormat:'.3s',}, resource_id, 0);

        const hno = results[0].filter(row => row.population_group === 'all' && row.population_status === 'INN');
        if (hno.length>0) updateKeyFigureData({title: 'People in Need', value: +hno[0].population}, hno[0].resource_hdx_id, 1);
        ...
        if (conflictData.length>0) updateKeyFigureData({title: 'Conflict Events in 2024', value: totalEvents}, conflictData[0].resource_hdx_id, 3);

        //risk
        const riskData = results[3][0];
        if (riskData) updateKeyFigureData({title: 'Overall Risk', value: riskData.overall_risk}, riskData.resource_hdx_id, 4);
  ...
  <div class='content grid-container'>
    {#if mapData.length>0 || chartData.length>0}
      <div class='sidebar col-5' bind:clientWidth={sidebarWidth}>
        ...
        {#if currentLayer.id==='population' && chartData.length<1}
          <div class='grid-container key-figure-container'>
            <div class='col-6'>
              <KeyFigure title={'Female Population'} value={chartData.femalePopulation} metadata={metadata} valueFormat={'.0%'} />
            </div>
            <div class='col-6'>
              <KeyFigure title={'Male Population'} value={chartData.malePopulation} metadata={metadata} valueFormat={'.0%'} />
            </div>
          </div>
        {:else if currentLayer.id==='orgs'}
